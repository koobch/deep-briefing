...
Step 2: Tension 해소 프로토콜
  ...
  2-d. 미해소 긴장 처리:
    ...
    - report-writer에 미해소 목록 전달 → 슬라이드에도 명시
...
```
1. report-slides.md의 각 섹션에 slide_meta 메타데이터가 자동 부여됨
...
3. PPT 생성 경로 (택 1):
   ...
   경로 B — python-pptx (로컬):
     ...
     b. report-slides.md의 slide_meta 메타데이터 파싱
     c. 템플릿 기반 PPTX 생성
...
| **Phase 3 → Phase 4** | `thinking-loop/loop-convergence.md`의 `converged: true` | 자동 진입 | 사용자 게이트 |
| **Phase 3 미수렴** | 사고 루프 2회 반복 후에도 `converged: false` | PM이 사용자에게 에스컬레이션 | 사용자에게 선택지 제시 (강제 수렴/추가 반복/방향 전환) |
| **Phase 4 → Phase 5** | `reports/report-docs.md` + `reports/report-slides.md` 생성 완료 | 자동 진입 | 사용자 게이트 |
