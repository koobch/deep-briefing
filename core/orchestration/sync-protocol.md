...
research_plan:
  ...
  divisions:
    market:
      active: true
      sub_leads: [geography-lead, genre-lead, platform-lead, competitive-lead]
      leaves: [east-asia-analyst, western-analyst, emerging-analyst, ...]
      priority_focus: "Client Brief에서 도출된 시장 분석 초점"

    product:
      active: true
      leaves: [game-design-analyst, art-ux-analyst, ip-analyst, user-analyst]
      priority_focus: "..."
    ...
    operations:
      active: false
      activation_criteria: "프로세스 효율화, 운영 최적화, 라이브 서비스 운영, 공급망이 핵심일 때"
...
[필수 검증]
  H-01: "카테고리A가 현재 역량으로 실현 가능한 유일한 옵션이다"
        ...
        반론: 카테고리B도 소규모 프로토로 검증 가능 (Product)
...
Step 2: Cross-domain 라우팅
  ...
  - 해당 Division에 라우팅:
    "market→product 질문: 카테고리B Top 10 제품의 설계 패턴은?"
    → product-lead에게 전달
...
tensions:
  - id: T-01
    ...
    between:
      - division: market
        claim: MGN-02
        position: "카테고리B CAGR 15%"
      - division: capability
        claim: CDP-01
        position: "카테고리B 개발 경험 0건"
...
action_items:
  - division: product
    action: "카테고리B Top 10 제품 설계 패턴 조사"
...
⚠️ 식별된 긴장:
- T-01: 시장 기회(카테고리B) vs 역량 부재(경험 0건)
...
PM 수렴 판정:
  - 논리 단절 0건 + Critical 블라인드 스팟 0건 + BASE 시나리오 자력 실현 가능 → 수렴
...
보고서의 각 콘텐츠 블록을 PPT 슬라이드 섹션에 매핑하고, 실제 PPT를 생성한다.

```
1. report-slides.md의 각 섹션에 slide_target 메타데이터가 자동 부여됨
   (report-writer가 작성 시 포함 — output-format.md의 슬라이드 매핑 규칙 참조)

2. PM이 사용자에게 슬라이드 구성 확인:
   📋 슬라이드 구성 (report-slides.md 기반)

   [슬라이드 1] 타이틀 — 핵심 결론 1문장 + 투자 규모 + ROI
   [슬라이드 2] 문제 정의 — 매출 공백 테이블 + 주력 제품 매출 급감
   [슬라이드 3] 시장 기회 — 장르×역량 매트릭스
   ...

   → 슬라이드 순서/포함 내용을 변경하시겠습니까?

3. PPT 생성 경로 (택 1):

   경로 A — Canva MCP (권장):
     a. PM이 Canva MCP로 디자인 생성 (generate-design-structured)
     b. 브랜드킷 적용 (list-brand-kits → 회사 템플릿)
     c. 슬라이드별 콘텐츠 삽입 (start-editing-transaction → perform-editing-operations)
     d. 최종 export (export-design → PPTX 또는 PDF)
     e. 산출물: {project}/reports/report-slides.pptx

   경로 B — python-pptx (로컬):
     a. PM이 scripts/generate-ppt.py 실행
     b. report-slides.md의 slide_target 메타데이터 파싱
     c. 템플릿 기반 PPTX 생성
     d. 산출물: {project}/reports/report-slides.pptx

   경로 C — 수동 (슬라이드 매핑만):
     a. report-slides.md를 슬라이드별 섹션으로 구조화하여 전달
     b. 사용자가 직접 PPT 작성 시 참조

4. Interactive/Team 모드: PPT 생성 전 슬라이드 구성 사용자 확인
   Auto 모드: 경로 A 자동 실행 (Canva 연결 시) 또는 경로 C
```
...
# 현재 상태
current_phase: "phase-id"           # 예: "0-client-discovery", "1-division-research", "sync-round-1"
...
보고서 품질을 기계적으로 보장하는 **자동 수정 루프**. 사람 개입 없이 2회 연속 PASS까지 반복한다.
...
2. qa-orchestrator가 6개 검증기를 순차 실행:
   ...
   d. confidence-prominence-checker: confidence vs 배치 위치 정합성
      - confidence: low/medium 수치가 Executive Summary, 슬라이드 전면, 핵심 논거에 사용되면 FAIL
      - 규칙: Executive Summary/슬라이드 1~3장의 핵심 수치는 confidence: high만 허용
      ...
      ```
      Step 1: 보고서에서 수치 추출
        report-docs.md + report-slides.md에서 숫자+단위 패턴 추출
      ...
      Step 3: confidence × 배치 위치 교차
        위치 분류:
          A등급 (고노출): Executive Summary, 슬라이드 1~3, 핵심 결론 1~3번
          B등급 (중노출): 본문 섹션 제목, 테이블 헤더, 슬라이드 4~8
          C등급 (저노출): 본문 상세, 부록, 별첨
   ...
   f. audience-fit-checker: 청중 적합성 검증
      - 슬라이드 수가 발표 시간 대비 적절한지 (1슬라이드 = 1.5~2분 기준)
...
루프 조건: qa-report에 Critical 또는 Major 이슈가 1건 이상
  ...
  │     - 입력: report-docs.md + report-slides.md │
...
[보고서 파일]
  - report-docs.md (상세 보고서)
  - report-slides.md (경영진 요약)
  - report-slides.pptx (PPT, 생성된 경우)
...
[피드백 가이드]
  ...
  예시 입력:
    "② Section 3.2 Merge 장르 분석이 얕다.
     벤치마크 타이틀 3개 이상 + 성공/실패 요인을 추가해줘."
...
[변경 사항]
  ...
  - FB-02: "Merge 장르 분석 심화 추가 (Section 3.2)"
...
Step 1: 영향 분석
  PM이 요청을 분석하여 다음을 판정:
    - 되돌아갈 Phase: 0 | 0.5 | 1 | Sync1 | 2 | Sync2 | 3 | 4 | 5
...
| **Phase 4 PPT** | 경로 A 자동 또는 C | 슬라이드 구성 확인 | 슬라이드 구성 토론 |
...
| Mid-Research 체크인 | 없음 | Phase 2 후, Phase 4 후 | Phase 2 후, Phase 3 후, Phase 4 후 |
...
│   │   ├── platform/
│   │   ├── competitive/
│   │   ├── opportunity-matrix.yaml   (market-lead: 지역×장르×플랫폼 기회 매트릭스)
│   │   └── division-synthesis.yaml   (market-lead)
│   ├── product/
│   │   ├── quick-scan.yaml            (Phase 0.5)
│   │   ├── game-design-analyst.yaml
│   │   ├── art-ux-analyst.yaml
│   │   ├── ip-analyst.yaml
│   │   ├── user-analyst.yaml
...
│   ├── report-slides.md
│   └── report-slides.pptx            (Phase 4-B: PPT 생성 시)
│
├── qa/
│   ├── qa-report.md                   (최종)
