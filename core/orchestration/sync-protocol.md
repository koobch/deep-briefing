...
### 순서 (3단계: Step 1 병렬 → Step 2 순차 → Step 3 통합)

```
Step 1: logic-prober + strategic-challenger (병렬 실행)
  ┌─────────────────────────────────────┬─────────────────────────────────────┐
  │ logic-prober (수직 검증)             │ strategic-challenger (수평 도전)      │
  │                                     │                                     │
  │ - cross-domain-synthesis의 핵심      │ - 대안 전략 생성 (최소 2개)           │
  │   인사이트/전략에 Why Chain          │ - 실패 시뮬레이션                     │
  │ - strategic_impact: high Claim에     │ - 경쟁자 대응                        │
  │   Why Chain (재귀)                   │ - 비대칭 사고                        │
  │ - 논리 단절 발견 시: 해당 Division에  │ - 포트폴리오 자기모순 탐지            │
  │   재검증 요청                        │                                     │
  │                                     │                                     │
  │ 산출물: why-probe.md                 │ 산출물: strategic-challenge.md        │
  └─────────────────────────────────────┴─────────────────────────────────────┘
...
Step 3: insight-synthesizer (통합 반영 — Step 1+2 모두 완료 후)
...
**병렬화 효과**: 기존 4단계 순차 → 3단계 (첫 단계 병렬). LP와 SC의 실행 시간이 중첩되어 Phase 3 전체 소요 시간 ~25-30% 단축.
**품질 영향 없음**: RT는 SC 결과를 참조해야 하므로 순차 유지. IS는 모든 선행 결과를 통합하므로 최후 실행.
...
### Step 4-B: 슬라이드 덱 생성 (선택적)

```
활성화 조건: Client Brief의 report_format이 "slides" 또는 "both"인 경우
미요청 시: 건너뛰고 Phase 4.5로 진행

1. PM이 slide-writer 스폰 (.claude/agents/slide-writer.md)
2. 입력:
   - {project}/reports/report-docs.md (Phase 4-A 산출물)
   - {project}/findings/golden-facts.yaml
   - {project}/00-client-brief.md
   - core/style/section-map.md (22개 슬라이드 유형)
   - core/style/slide-system.css
   - core/style/design-guide.md
3. 산출물:
   - {project}/reports/slides/slide-deck.html (슬라이드 덱)
   - {project}/reports/slides/slide-outline.yaml (구성 메타데이터)
4. 슬라이드 수 가이드: 1분당 1~1.5장 (Client Brief 발표 시간 기반)
```
...
| **Phase 4 → Phase 4.5** | `reports/report-docs.md` 생성 완료 (+ 슬라이드 요청 시 `reports/slides/` 존재, 원페이퍼 요청 시 `reports/one-pager.md` 존재) | 자동 진입 | 사용자 게이트 |
| **Phase 4.5 → Phase 5** | `source-registry.csv` 생성 완료 (`scripts/generate-source-registry.py` 실행) | 자동 진입 | 사용자 게이트 |
