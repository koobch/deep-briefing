...
| **Phase 3.7 → Phase 4-A** | `self-critique.md` 존재 (Auto: FLAG 0~1건이면 면제) | 자동 진입 | 사용자 게이트 |
| **Phase 3 미수렴** | 사고 루프 2회 반복 후에도 `converged: false` | PM이 사용자에게 에스컬레이션 | 사용자에게 선택지 제시 (강제 수렴/추가 반복/방향 전환) |
| **Phase 4-A → Phase 4-C** | `reports/report-docs.md` 생성 완료 | 자동 진입 (원페이퍼 요청 시) | 사용자 게이트 |
| **Phase 4-C → Phase 4.5** | 원페이퍼 요청 시 `reports/one-pager.md` 존재 (미요청 시 건너뜀) | 자동 진입 | 사용자 게이트 |
