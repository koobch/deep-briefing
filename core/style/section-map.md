...
### 유형별 상세 밀도 (1920×1080 기준, element-catalog.md 섹션 4.1 BCG 실측 기반)

| 유형 | 필수 콘텐츠 블록 | 차트/데이터 항목 | 텍스트 영역 | 보조 요소 | 최소 줄 수 |
|------|------------|-------------|---------|---------|---------|
| Exec Summary | 3행 × (리드1+불릿5+마무리1) | 행당 리드1 + 불릿5 | 3행 균등 분할 | 마무리 문장 | **21줄** |
| Insight+Chart | 차트6행 + 패널4문단(16줄) + callout2줄 | 차트 5~6행 | 패널 4~5문단 | callout 1개 | **24줄** |
| Ranked | 5항목×(제목+불릿2) + 패널3문단 + 그리드 | 아이콘 5개 | 패널 4문단 | 하단 산업별 그리드 | **25줄** |
| Big Number | KPI2개(6줄) + 차트5행 + 해석3줄 + callout2줄 | KPI 2~3개 | 차트 4~5행 | spread-2 하단 | **20줄** |
| Process | 좌 불릿8개 + 우 6행 테이블 + 쉐브론3×3 | 좌 불릿 **8개** | 우 **6행 테이블** | 쉐브론 3단계 | **30줄** |
| Before/After | — | 3컬럼 × 4행 | Impact Big Number 3개 | 각 행 2줄+ | — |
| Table/List | 6행 × (제목+설명2줄) | 행 **6개** | 각 행 bold + 상세 2줄 | 아이콘 | **18줄** |
| Case Study | — | Impact 3개 | 플랫폼 3개 | 운영모델 3개 | — |
| Comparison | 좌우 각 (제목+설명+불릿6) | 좌우 각 **6불릿** | 섹션 제목 | 헤더 대비 | **20줄** |
| Multi-Column | 3컬럼 × (바4+불릿3×2줄) | 컬럼당 바 4~5개 | 불릿 3~4개 | 지역 헤더 | **30줄** |
| Split Layout | 좌(제목+설명3줄+불릿5) + 우(바5+annotation) | 좌 불릿 5개+ | 우 바 5~6개 | 좌 callout | **22줄** |
...
| Matrix 2×2 | 4셀 × (제목+설명3줄+불릿2) | 4셀 내용 각 3줄 | 축 라벨 | 헤더 | **24줄** |
| Framework | 4항목 × (제목+HOW설명2줄) + 링 + callout | 링 4라벨 | 범례 4항목 각 1~2줄 | 중심 텍스트 | **18줄** |
| Introduction | 3문단(12줄) + 질문4개 + 종합2줄 + 사이드바 | 좌 문단 3개 + 불릿 4개 | 우 사이드바 5항목 | bold 종합 문장 2줄 | **22줄** |

> **공간 채우기 5원칙**:
> 1. 콘텐츠가 slot-body 수직 공간의 **80% 이상**을 차지해야 한다
> 2. 연속 빈 공간 **80px 이상 금지** — 빈 공간 발견 시 annotation, callout, 보조 통계 추가
> 3. 빈 공간으로 남기기 금지 — 콘텐츠가 부족하면 **리서치를 되돌아가서 보강**한다
> 4. 모든 비면제 슬라이드에 하단 채우기 요소(callout/annotation/read-more) **최소 1개 필수**
> 5. 슬라이드당 총 텍스트 **25줄 이상**, 불릿 **8개 이상** (Exec Summary, Table 등 구조형 유형은 행×항목 기준)
