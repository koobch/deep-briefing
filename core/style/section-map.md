...
- **밀도**: 행당 리드 1줄 + 불릿 3~5개 + 마무리 1줄 = **최소 5줄**
...
|------|------|------|------|-----------|
| S-SUBQ | `--fs-subq` (:lang(ko) 28px), bold | 좌 | `var(--c-text-sub)` (#03522d) | 질문형 |
| S-CHART | — | flex:1 | — | 차트 항목 최소 4개, 데이터 라벨 필수 |
...
| S-PANEL-BODY | `--fs-body` (:lang(ko) 22px) | 좌 | `var(--c-text)` (#000000) | 3~5문단, bold 핵심구 |
...
| 우측 패널 | 최소 3문단 (문단당 bold 1~2개) |
| 차트 | 항목 최소 4개, 데이터 라벨 필수 |
...
- **최소 입력**: 행 6개+ × 열 4개+, 각 셀에 방향성(증가/감소) 또는 수치
...
- **최소 입력**: 점 8개+, X축/Y축 연속형 수치, 각 점에 라벨
...
- **최소 입력**: 2개 축 정의 + 셀 4개(2×2) 이상, 각 셀에 항목명+설명
