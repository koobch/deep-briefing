...
- **오케스트레이션 흐름**: Phase 0 → 0.5(가설+데이터갭) → 1(병렬 리서치) → Sync 1 → 2(심화) → Sync 2(긴장해소) → 3(사고 루프+Red Team) → 3.7(External Review) → 4-A(세로 보고서) → 4-B(슬라이드, 선택적) → 4.5(출처 레지스트리) → 5(QA 자동 루프) → 5.5(피드백)
...
## 에이전트 인벤토리 (총 48: 비Leaf 22 + Leaf 26)
...
| Report | report-writer, slide-writer | 2 |
