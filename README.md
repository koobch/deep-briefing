...
| **SCR 스토리라인** | Situation→Complication→Resolution + 모든 보고서 섹션 Action Title 필수 |
...
| **tmux** | Division 병렬 실행 (모드 A) | 조건부 — 병렬 실행(모드 A)에 필요. 없으면 순차 실행(모드 B)으로 자동 전환 |
...
```
my-first-research/
├── reports/report-docs.md      ← 전략 보고서 (경영진 요약 포함)
...
리서치 1회 실행 시 예상 토큰 사용량과 비용입니다. 주제 복잡도, Division 수, 모드에 따라 달라집니다.

| 구성 | Division 수 | 예상 토큰 | 예상 비용 (Opus) | 소요 시간 |
|------|-----------|----------|----------------|----------|
| **Express (최소)** | 핵심 4 | ~1M | ~$15~25 | 30분~1시간 |
| **Standard (핵심)** | 핵심 4 | ~2~3M | ~$30~60 | 1~2시간 |
| **Full (전체)** | 전체 7 | ~4~6M | ~$60~120 | 2~3시간 |

> **비용 절감 팁**: Auto 모드가 Interactive보다 토큰을 ~30% 적게 사용합니다.
> API 키를 설정하면 웹 검색 횟수가 줄어 토큰이 절약됩니다.
> Sonnet 모델로 전환하면 비용이 ~1/5로 줄지만 분석 깊이가 낮아집니다.
