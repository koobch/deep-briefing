...
부트스트랩 시 아래를 읽어라:
- `core/knowledge/common-sense.yaml` — 범용 분석 상식 (Layer 0)- `domains/{domain}/knowledge/learned-sources.yaml` — 인프라 데이터 소스
...
- **company_profile_addons** (entity_type=company):
  - entity_type=company 시: 인프라 투자 계획·현황
- **iteration_log 기록 의무**: `baseline_area: "기술·물리 인프라"` + `deliverable_status: {각 항목: complete|partial|unavailable}`
