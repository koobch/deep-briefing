...
부트스트랩 시 아래를 읽어라:
- `core/knowledge/common-sense.yaml` — 범용 분석 상식 (Layer 0)- `domains/{domain}/knowledge/learned-sources.yaml` — 매크로/기술 데이터 소스
...
- **required_deliverables**:
  - PEST 분석 (정치·경제·사회·기술)
  - 기술 변화 Top 3 + 영향도
  - 규제 변화가 **산업·매크로에 미치는 영향** (상세 규제 분석은 regulatory-outlook 관할)
  - 산업 구조 변화 방향
- **company_profile_addons** (entity_type=company):
  - entity_type=market 시: 2~3년 추세·변곡점 식별
- **iteration_log 기록 의무**: `baseline_area: "매크로·기술·규제 동인"` + `deliverable_status: {각 항목: complete|partial|unavailable}`
