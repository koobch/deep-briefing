...
부트스트랩 시 아래를 읽어라:
- `core/knowledge/common-sense.yaml` — 범용 분석 상식 (Layer 0)- `domains/{domain}/knowledge/learned-sources.yaml` — 제품 정보 소스
...
- **company_profile_addons** (entity_type=company):
  - entity_type=company 시: 주력 제품 vs 실험 제품 구분, 로드맵 단서
- **iteration_log 기록 의무**: `baseline_area: "제품·서비스 포트폴리오"` + `deliverable_status: {각 항목: complete|partial|unavailable}`
