...
부트스트랩 시 아래를 읽어라:
- `core/knowledge/common-sense.yaml` — 범용 분석 상식 (Layer 0)- `domains/{domain}/knowledge/learned-sources.yaml` — 가격/수익 데이터 소스
...
- **required_deliverables**:
  - 가격 구조 (구독/종량/프리미엄)
  - 수익 단위 (unit economics)
  - 가격 탄력성 추정
  - 수익 모델 다각화
- **company_profile_addons** (entity_type=company):
  - entity_type=company 시: 공시 기준 매출/가입자/ARPU
- **iteration_log 기록 의무**: `baseline_area: "수익 모델·가격"` + `deliverable_status: {각 항목: complete|partial|unavailable}`
