...
부트스트랩 시 아래를 읽어라:
- `core/knowledge/common-sense.yaml` — 범용 분석 상식 (Layer 0)- `domains/{domain}/knowledge/learned-sources.yaml` — 고객 인사이트 소스
...
- **required_deliverables**:
  - JTBD 매칭 점수
  - 경쟁사 대비 차별화 요소 3~5개
  - PMF 지표 (NPS/Retention/ARPU)
  - 핵심 Value Prop 1문장
- **company_profile_addons** (entity_type=company):
  - entity_type=company 시: 사용자 리뷰 기반 차별화 검증
- **iteration_log 기록 의무**: `baseline_area: "가치 제안·차별화"` + `deliverable_status: {각 항목: complete|partial|unavailable}`
