...
부트스트랩 시 아래를 읽어라:
- `core/knowledge/common-sense.yaml` — 범용 분석 상식 (Layer 0)- `domains/{domain}/knowledge/learned-sources.yaml` — 규제 데이터 소스
...
- **company_profile_addons** (entity_type=company):
  - entity_type=company 시: 최근 규제 이슈·조사 이력
- **iteration_log 기록 의무**: `baseline_area: "적용 규제·준수 현황"` + `deliverable_status: {각 항목: complete|partial|unavailable}`
