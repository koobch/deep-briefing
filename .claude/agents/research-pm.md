...
- 사고 루프 수렴 판정 (logic-prober → strategic-challenger → insight-synthesizer)
...
- 사고 루프 실행 (질문/도전/통합) → logic-prober, strategic-challenger, insight-synthesizer
...
질문 5번 답변 후 반드시 후속 질문:
  ...
  5-c. "데이터의 출처와 기간은? (예: Sensor Tower 2021~2025)"
...
research_plan:
  ...
  domain: {active-domain}        # 예: game-strategy, healthcare, fintech
  ...
  divisions:
    ...
    operations:
      active: false
      activation_criteria: "프로세스 효율화, 운영 최적화, 라이브 서비스 운영"
  ...
  ep_warnings:
    - ep: EP-027
      target: [art-ux-analyst, product-lead, strategic-challenger]
      message: "제품-장르 적합도 검증 필수"
...
1. Research Plan의 divisions + data_sources를 분석하여 필요 API 목록 도출:
   ...
   │ PC/Steam 게임 분석 포함       │ STEAM_API_KEY        │ 권장   │
...
```
1. data-preprocessor (유틸리티 에이전트) 스폰 (Agent 도구)
...
```
Sync Round 1 후 사용자에게 브리핑:

📋 Sync Round 1 결과

[Division별 핵심 발견 요약 — headline 4개]
...
순서 고정: logic-prober → strategic-challenger → insight-synthesizer
...
Step 3: insight-synthesizer 스폰 (Agent 도구)
  입력: cross-domain-synthesis + why-probe + strategic-challenge
  수행: 도전 결과 반영 → 전략 보강/수정
...
report-writer 스폰 (Agent 도구)
  입력:
    ...
    - thinking-loop 결과 (why-probe + strategic-challenge + loop-convergence)
    - 전체 Division 출력 (필요 시 드릴다운)
    - Client Brief (톤, 형식 선호)
  산출물:
...
Step 1: qa-orchestrator 스폰 (Agent 도구)
  qa-orchestrator가 내부적으로:
    a. mechanical-validator 실행 (수치 정합성, 형식)
       → python scripts/verify-facts.py {project} 자동 실행 → qa/fact-verification.yaml 생성
    b. source-traceability-checker 실행 (소스 추적)
    b-2. source-url-verifier 실행 (URL 접근성 L1 + 관련성 L2)
    c. report-auditor 실행 (논리 완결성, So What 테스트)
    d. 이슈 발견 시 → report-fixer 최소 수정 → 재검증
    e. 2회 연속 clean pass까지 반복
...
| EP-027 (제품-장르 적합도) | 사고 루프 수렴 판정 시 Critical 플래그 잔존 여부 확인 |
...
| insight-synthesizer | Phase 3 | 전체 사고 루프 입력 | `thinking-loop/loop-convergence.md` |
