...
2. 에이전트 정의 탐색 우선순위:
   ...
   c. `.claude/agents/leaves/product/{leaf-id}.md` (범용 Leaf 역할 정의 + 내부 MECE 분석 구조)
   d. 정의 파일 없음 → Agent 도구 prompt에 역할을 직접 기술하여 동적 스폰
      (상세: `core/templates/division-lead-template.md` "동적 스폰" 참조)
3. 전체 워크플로우 실행:
   - Leaf 4명 스폰 (product-offering, value-differentiation, go-to-market, pricing-monetization)
