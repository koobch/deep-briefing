...
Step 1: 레인 1 — 대안 전략 생성
  ...
  대안 품질 최소 기준 (미달 시 재작성):
  ☐ 실행 주체가 식별 가능 (누가 하는가?)
  ☐ 타임라인 윤곽이 있음 (언제까지?)
  ☐ 현행 전략 대비 최소 1개 핵심 가정이 다름
  ☐ "현행보다 나을 수 있는 조건"이 명시됨
...
Step 5: 레인 5 — 포트폴리오 자기모순 탐지
  ...
  ※ insight-synthesizer의 "BASE 시나리오 자력 실현" 판정과의 차이:
    - strategic-challenger Lane 5: 전략 요소 간 "상충 구조" 발견 (입력 역할)
    - insight-synthesizer: 상충 발견 후 "해소 가능 여부" 판정 (판단 역할)
  → Lane 5는 상충을 발견하여 insight-synthesizer에 넘긴다. 해소 판정은 하지 않는다.
