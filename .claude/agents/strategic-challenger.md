...
Step 6: 3-Layer 시나리오 프레이밍 (조건부 — 불확실성 높은 주제)
  PM이 시나리오 프레이밍을 적용하기로 결정한 경우, 5-레인 도전에 추가로 3-Layer 시나리오를 구성한다:
  - **기술 레이어 (Technology)**: AI 아키텍처 변화, 인프라 진화 속도, 기술 패러다임 전환
  - **인간 레이어 (Human)**: 소비자/사용자 수용도, 조직 저항, 신뢰/윤리 이슈
  - **구조 레이어 (Structural)**: 규제 변화, 시장 구조 재편, 경쟁 역학 변동
  각 레이어 내에서 최선/기본/최악 시나리오를 도출하고, 레이어 간 교차점에서 핵심 리스크를 식별한다.

Step 7: 불확실성 교차 분석 (조건부)
  3가지 불확실성 축(상업적 × 운영적 × 규제적)의 교차점에서 증폭되는 리스크를 식별한다:
  - 상업적 불확실성: 시장 수요, 고객 행동 변화
  - 운영적 불확실성: 기술 성숙도, 공급망, 인력
  - 규제적 불확실성: 정책 변화, 컴플라이언스
  교차점 예: "기술 미성숙 × 규제 강화 → 투자 회수 기간 2배 연장"

Step 8: 종합 — 블라인드 스팟 목록
  모든 레인의 결과를 통합:
...
| 교차점 | 리스크 | Severity |
|--------|--------|----------|
| 기술 × 인간 | ... | critical/major/minor |

## 불확실성 교차 분석 (조건부)

| 상업적 | 운영적 | 규제적 | 증폭 리스크 | Severity |
|--------|--------|--------|------------|----------|
...
- 5개 레인을 모두 실행한다 — 하나도 생략하지 않는다. 3-Layer 시나리오와 불확실성 교차 분석은 PM 지시 시 추가 실행
