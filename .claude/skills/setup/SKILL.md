...
1. 자동 점검 + 설치 — 필요한 도구를 확인하고 설치합니다 (추천)
2. 점검만 — 현재 환경 상태만 확인하고, 설치는 하지 않습니다
3. 전부 스킵 — 환경 점검 없이 바로 도메인 설정으로 넘어갑니다
   (Python/tmux가 이미 있다면 3번도 괜찮습니다)
```

- 1 선택 시 → 아래 항목을 순서대로 점검 + 설치 제안
- 2 선택 시 → 아래 항목을 점검만 하고 설치는 건너뜀 (결과만 보여줌)
- 3 선택 시 → Phase 1 전체 스킵, Phase 2로 이동

**1 또는 2를 선택한 경우:**

다음 항목을 순서대로 Bash 도구로 점검한다. 각 항목에 대해 **있는지 → 없으면 어떻게 설치하는지 → 설치 후 확인**까지 완결한다. (2번 선택 시 "설치할까요?" 질문을 생략하고 상태만 보여준다.)
