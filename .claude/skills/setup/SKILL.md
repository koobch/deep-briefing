...
3. API = 전부 스킵 (웹 검색만으로 진행). 웹 검색만으로 충분한 리서치가 가능합니다. API는 나중에 /setup으로 추가할 수 있습니다.
...
AskUserQuestion 도구로 선택지를 제시하세요:

```yaml
AskUserQuestion:
  questions:
    - question: "환경 설정 방식을 선택해주세요."
      header: "환경 점검"
      multiSelect: false
      options:
        - label: "자동 점검 + 설치 (추천)"
        ...
        - label: "점검만"
          description: "상태 확인만 (설치는 직접 하세요)"
        - label: "전부 스킵"
          description: "환경 점검 없이 바로 도메인 설정으로 넘어갑니다"
```

- "자동 점검 + 설치" 선택 시 → 아래 항목을 순서대로 점검 + 설치 제안
- "점검만" 선택 시 → 아래 항목을 점검만 하고 설치는 건너뜀 (결과만 보여줌)
- "전부 스킵" 선택 시 → Phase 1 전체 스킵, Phase 2로 이동

**"자동 점검 + 설치" 또는 "점검만"을 선택한 경우:**

다음 항목을 순서대로 Bash 도구로 점검한다. 각 항목에 대해 **있는지 → 없으면 어떻게 설치하는지 → 설치 후 확인**까지 완결한다. ("점검만" 선택 시 "설치할까요?" 질문을 생략하고 상태만 보여준다.)
...
3. **tmux**:
   ...
   - ❌ 없음 → 사용자에게 설명:
     ...
     없으면 TeamCreate 모드(단일 CLI)로도 리서치 가능하지만, Division이 순차 실행되어 리서치 시간이 약 2배 길어질 수 있습니다.
...
tmux가 미설치인 경우: "tmux가 없으면 모드 B(순차 실행)로 자동 전환됩니다. 성능은 줄지만 결과 품질은 동일합니다."

**⚠️ 항목이 있으면**: "문제가 있는 항목이 있다면, 해결 후 /setup을 다시 실행하세요. 이전 설정은 유지됩니다."
...
**도메인 선택** — AskUserQuestion 도구로 선택지를 제시하세요:

```yaml
AskUserQuestion:
  questions:
    - question: "어떤 도메인으로 시작할까요?"
      header: "도메인"
      multiSelect: false
      options:
        - label: "범용 전략 분석 (추천)"
          description: "산업 무관, 바로 시작 가능. 나중에 산업 특화 도메인 추가 가능"
        - label: "산업 직접 지정"
          description: "분석할 산업을 입력합니다 (예: gaming, healthcare, fintech)"
```

- "범용 전략 분석" 선택 시 → example 도메인 사용, Step 2-A~D 전체 스킵, Step 2-E 요약만 출력 후 Phase 3 진행
- "산업 직접 지정" 또는 Other로 산업명 입력 시 → 아래 Step 2-A부터 정상 진행 (입력한 산업 기반으로 도메인 생성)
...
| 투자/금융 | DCF/멀티플 밸류에이션, 포트폴리오 이론, 리스크 팩터 분석, ESG 스코어링 |

위 목록에 없는 산업이면 Claude가 해당 산업에 맞는 프레임워크를 자동으로 추천합니다.
  ...
  questions:
    - question: "API 키를 설정하시겠습니까?"
      ...
      options:
        ...
        - label: "전부 스킵"
          description: "API 설정을 건너뛰고 바로 리서치를 시작합니다"
```

- "나중에 설정" 또는 "전부 스킵" 선택 시 → Step 2-C 전체 스킵, Phase 3 진행
- "지금 설정" 선택 시 → 아래 API 추천 + 설정 플로우 진행
...
산업 특화 API 추천 시, 자동 탐색이나 직접 입력으로 이미 설정된 API는 '✅ 이미 설정됨'으로 표시하고 재설정을 묻지 않는다.
...
1. 사용자 입력에서 주제를 파악하고, 주제에서 프로젝트명을 자동 생성한다
   - 예: "한국 SaaS 시장 진출 전략" → 프로젝트명 `saas-korea-market`
   - 사용자에게 확인: "프로젝트명을 'saas-korea-market'으로 생성했습니다. 이대로 진행할까요? (다른 이름을 원하면 입력해주세요)"
   - 사용자가 다른 이름을 입력하면 해당 이름 사용, 확인(Y/응)하면 자동 생성명 사용
