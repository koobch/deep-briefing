...
1. **Interactive** (추천) — 매 단계 확인하며 함께 진행. 가설 검토, 중간 브리핑 포함
2. **Auto** — 전 과정을 자동으로 진행. 결과만 받고 싶을 때
3. **Team** — 심층 토론 + 전방위 되돌아가기 지원. 복잡한 주제에 적합
...
모드 없이 주제만 입력한 경우에도 반드시 선택지를 먼저 보여준 뒤, 사용자가 선택하면 해당 모드 + 입력된 주제로 이어서 진행한다.
