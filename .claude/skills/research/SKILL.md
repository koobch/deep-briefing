...
# v4.11: Analysis Type 명시 (선택)
/research auto --type profile [주제]                 → 기업·시장 전방위 스터디
/research auto --type decision [주제]                → 의사결정 지원 (기본)
/research auto --type exploration [주제]             → 기회 탐색·트렌드
/research auto --type monitoring [주제]              → 지속 관찰·지표
```

### Analysis Type 자동 판정 (v4.11)
PM이 Phase 0-A.6에서 주제 키워드로 타입을 자동 판정한다. CLI 플래그로 명시하지 않아도 됨:
- 특정 기업/제품 명사 + "분석/프로파일" → **profile**
- "~할까/진출/투자/M&A" → **decision**
- "트렌드/기회/신성장" → **exploration**
- "분기별/월간/업데이트" → **monitoring**
- 기본값: decision
